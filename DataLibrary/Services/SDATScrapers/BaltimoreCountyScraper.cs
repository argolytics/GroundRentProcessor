...
public class BaltimoreCountyScraper : IRealPropertySearchScraper
{
    ...
    private readonly BaltimoreCountyDataServiceFactory _baltimoreCountyDataServiceFactory;
    ...
    public BaltimoreCountyScraper(
        ...
        BaltimoreCountyDataServiceFactory baltimoreCountyDataServiceFactory)
    {
        _dataContext = dataContext;
        _baltimoreCountyDataServiceFactory = baltimoreCountyDataServiceFactory;
    }
        ...
        int amountToScrape)
    {
        ...
        {
            var baltimoreCountyDataService = _baltimoreCountyDataServiceFactory.CreateGroundRentProcessorDataService(uow);
            //AddressList = await baltimoreCountyDataService.ReadTopAmountWhereIsGroundRentNull(amountToScrape);
            AddressList = await baltimoreCountyDataService.ReadTopAmountWhereIsGroundRentTrue(amountToScrape);
        }
        currentCount = 0;
        ...
        try
        {
            ...
            foreach (var address in iterList)
            {
                ...
                Input = webDriverWait.Until(ExpectedConditions.ElementToBeClickable(By.CssSelector("#cphMainContentArea_ucSearchType_wzrdRealPropertySearch_ucSearchType_ddlCounty > option:nth-child(5)")));
                Input.Click();
                ...
                if (firefoxDriver.FindElements(By.CssSelector("#cphMainContentArea_ucSearchType_lblErr")).Count != 0)
                {
                        ...
                        .Text.Contains("There are no records that match your criteria"))
                    {
                        ...
                        {
                            var baltimoreCountyDataService = _baltimoreCountyDataServiceFactory.CreateGroundRentProcessorDataService(uow);
                            dbTransactionResult = await baltimoreCountyDataService.Delete(address.AccountId);
                        }
                ...
                else
                {
                    ...
                    if (firefoxDriver.FindElements(By.CssSelector("#cphMainContentArea_ucSearchType_wzrdRealPropertySearch_ucGroundRent_lblErr")).Count != 0)
                    {
                            ...
                            .Text.Contains("There is currently no ground rent"))
                        {
                            ...
                            {
                                var BaltimoreCountyDataService = _baltimoreCountyDataServiceFactory.CreateGroundRentProcessorDataService(uow);
                                dbTransactionResult = await BaltimoreCountyDataService.CreateOrUpdateSDATScraper(new AddressModel
                                {
                    ...
                    else
                    {
                        ...
                        if (webDriverWait.Until(d => ((IJavaScriptExecutor)d).ExecuteScript("return document.readyState").Equals("complete")))
                        {
                            ...
                            pdf.SaveAsFile($@"C:\Users\Jason\Desktop\GroundRentRegistrationPdfs\BACO\{accountId}.pdf");
                            pdfDownloaded = true;
                        }
