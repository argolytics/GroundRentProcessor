...
public class CalvertCountySqlDataService : IGroundRentProcessorDataService
{
    private readonly IUnitOfWork _unitOfWork;

    public CalvertCountySqlDataService(IUnitOfWork unitOfWork)
    {
    ...
    public async Task CreateOrUpdateFile(AddressModel addressModel)
    {
        ...
        await _unitOfWork.Connection.ExecuteAsync("spCalvertCounty_CreateOrUpdateFile", parms,
    ...
    public async Task CreateOrUpdateSDATRedeemedFile(AddressModel addressModel)
    {
        ...
        await _unitOfWork.Connection.ExecuteAsync("spCalvertCounty_CreateOrUpdateSDATRedeemedFile", parms,
            commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction);
    }
    public async Task<bool> CreateOrUpdateSDATScraper(AddressModel addressModel)
    {
        try
        {
            ...
            await _unitOfWork.Connection.ExecuteAsync("spCalvertCounty_CreateOrUpdateSDATScraper", parms,
                commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction);
            return true;
        }
        catch (Exception ex)
    ...
    {
        return (await _unitOfWork.Connection.QueryAsync<AddressModel>("spCalvertCounty_ReadTopAmountWhereIsGroundRentNull",
            new { Amount = amount },
    ...
    {
        return (await _unitOfWork.Connection.QueryAsync<AddressModel>("spCalvertCounty_ReadTopAmountWhereIsGroundRentTrue",
            new { Amount = amount },
            commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction)).ToList();
    }
    public async Task<bool> Delete(string accountId)
    {
        try
        {
            await _unitOfWork.Connection.ExecuteAsync("spCalvertCounty_Delete", new { AccountId = accountId },
            commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction);
            return true;
        }
        catch (Exception ex)
