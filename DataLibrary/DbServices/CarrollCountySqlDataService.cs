...
public class CarrollCountySqlDataService : IGroundRentProcessorDataService
{
    private readonly IUnitOfWork _unitOfWork;

    public CarrollCountySqlDataService(IUnitOfWork unitOfWork)
    {
    ...
    public async Task CreateOrUpdateFile(AddressModel addressModel)
    {
        ...
        await _unitOfWork.Connection.ExecuteAsync("spCarrollCounty_CreateOrUpdateFile", parms,
    ...
    public async Task CreateOrUpdateSDATRedeemedFile(AddressModel addressModel)
    {
        ...
        await _unitOfWork.Connection.ExecuteAsync("spCarrollCounty_CreateOrUpdateSDATRedeemedFile", parms,
            commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction);
    }
    public async Task<bool> CreateOrUpdateSDATScraper(AddressModel addressModel)
    {
        try
        {
            ...
            await _unitOfWork.Connection.ExecuteAsync("spCarrollCounty_CreateOrUpdateSDATScraper", parms,
                commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction);
            return true;
        }
        catch (Exception ex)
    ...
    {
        return (await _unitOfWork.Connection.QueryAsync<AddressModel>("spCarrollCounty_ReadTopAmountWhereIsGroundRentNull",
            new { Amount = amount },
    ...
    {
        return (await _unitOfWork.Connection.QueryAsync<AddressModel>("spCarrollCounty_ReadTopAmountWhereIsGroundRentTrue",
            new { Amount = amount },
            commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction)).ToList();
    }
    public async Task<bool> Delete(string accountId)
    {
        try
        {
            await _unitOfWork.Connection.ExecuteAsync("spCarrollCounty_Delete", new { AccountId = accountId },
            commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction);
            return true;
        }
        catch (Exception ex)
