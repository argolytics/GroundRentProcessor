...
public class GarrettCountySqlDataService : IGroundRentProcessorDataService
{
    private readonly IUnitOfWork _unitOfWork;

    public GarrettCountySqlDataService(IUnitOfWork unitOfWork)
    {
    ...
    public async Task CreateOrUpdateFile(AddressModel addressModel)
    {
        ...
        await _unitOfWork.Connection.ExecuteAsync("spGarrettCounty_CreateOrUpdateFile", parms,
    ...
    public async Task CreateOrUpdateSDATRedeemedFile(AddressModel addressModel)
    {
        ...
        await _unitOfWork.Connection.ExecuteAsync("spGarrettCounty_CreateOrUpdateSDATRedeemedFile", parms,
            commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction);
    }
    public async Task<bool> CreateOrUpdateSDATScraper(AddressModel addressModel)
    {
        try
        {
            ...
            await _unitOfWork.Connection.ExecuteAsync("spGarrettCounty_CreateOrUpdateSDATScraper", parms,
                commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction);
            return true;
        }
        catch (Exception ex)
    ...
    {
        return (await _unitOfWork.Connection.QueryAsync<AddressModel>("spGarrettCounty_ReadTopAmountWhereIsGroundRentNull",
            new { Amount = amount },
    ...
    {
        return (await _unitOfWork.Connection.QueryAsync<AddressModel>("spGarrettCounty_ReadTopAmountWhereIsGroundRentTrue",
            new { Amount = amount },
            commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction)).ToList();
    }
    public async Task<bool> Delete(string accountId)
    {
        try
        {
            await _unitOfWork.Connection.ExecuteAsync("spGarrettCounty_Delete", new { AccountId = accountId },
            commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction);
            return true;
        }
        catch (Exception ex)
