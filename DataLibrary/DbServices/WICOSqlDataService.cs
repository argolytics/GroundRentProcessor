...
public class WICOSqlDataService : IGroundRentProcessorDataService
{
    private readonly IUnitOfWork _unitOfWork;
    private const string _CreateAddressSql = @"INSERT INTO dbo.[WICO] ([AccountId], [County], [AccountNumber], [Ward], [Section], [Block], [Lot], [LandUseCode], [YearBuilt], [IsGroundRent], [IsRedeemed], [PdfCount], [AllDataDownloaded]) VALUES (@AccountId, @County, @AccountNumber, @Ward, @Section, @Block, @Lot, @LandUseCode, @YearBuilt, @IsGroundRent, @IsRedeemed, @PdfCount, @AllDataDownloaded); SELECT CAST(SCOPE_IDENTITY() AS INT);";

    public WICOSqlDataService(IUnitOfWork unitOfWork)
    {
