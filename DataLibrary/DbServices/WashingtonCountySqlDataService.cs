...
public class WashingtonCountySqlDataService : IGroundRentProcessorDataService
{
    private readonly IUnitOfWork _unitOfWork;

    public WashingtonCountySqlDataService(IUnitOfWork unitOfWork)
    {
    ...
    public async Task CreateOrUpdateFile(AddressModel addressModel)
    {
        ...
        await _unitOfWork.Connection.ExecuteAsync("spWashingtonCounty_CreateOrUpdateFile", parms,
    ...
    public async Task CreateOrUpdateSDATRedeemedFile(AddressModel addressModel)
    {
        ...
        await _unitOfWork.Connection.ExecuteAsync("spWashingtonCounty_CreateOrUpdateSDATRedeemedFile", parms,
            commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction);
    }
    public async Task<bool> CreateOrUpdateSDATScraper(AddressModel addressModel)
    {
        try
        {
            ...
            await _unitOfWork.Connection.ExecuteAsync("spWashingtonCounty_CreateOrUpdateSDATScraper", parms,
                commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction);
            return true;
        }
        catch (Exception ex)
    ...
    {
        return (await _unitOfWork.Connection.QueryAsync<AddressModel>("spWashingtonCounty_ReadTopAmountWhereIsGroundRentNull",
            new { Amount = amount },
    ...
    {
        return (await _unitOfWork.Connection.QueryAsync<AddressModel>("spWashingtonCounty_ReadTopAmountWhereIsGroundRentTrue",
            new { Amount = amount },
            commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction)).ToList();
    }
    public async Task<bool> Delete(string accountId)
    {
        try
        {
            await _unitOfWork.Connection.ExecuteAsync("spWashingtonCounty_Delete", new { AccountId = accountId },
            commandType: CommandType.StoredProcedure, transaction: _unitOfWork.Transaction);
            return true;
        }
        catch (Exception ex)
